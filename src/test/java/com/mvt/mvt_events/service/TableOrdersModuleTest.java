...
class TableOrdersModuleTest {
    ...
    // ================================================================
    // ClientWaiterService tests
    // ================================================================

    @Nested
    @DisplayName("ClientWaiterService")
    class WaiterServiceTests {

        @Mock private ClientWaiterRepository clientWaiterRepository;
        @Mock private UserRepository userRepository;
        @InjectMocks private ClientWaiterService waiterService;
        ...
        private User makeWaiter() {
            ...
            u.setUsername("joao@test.com");
            u.setRole(User.Role.WAITER);
            return u;
        }

        @Test
        @DisplayName("Vincular garçom ao estabelecimento")
        void vincularGarcom() {
            when(userRepository.findById(clientId)).thenReturn(Optional.of(makeClient()));
            when(userRepository.findById(waiterId)).thenReturn(Optional.of(makeWaiter()));
            when(clientWaiterRepository.existsByClientIdAndWaiterId(clientId, waiterId)).thenReturn(false);
            when(clientWaiterRepository.save(any())).thenAnswer(inv -> {
                ClientWaiter cw = inv.getArgument(0);
                cw.setId(1L);
                return cw;
            });

            ClientWaiter cw = waiterService.link(clientId, waiterId, "1234");

            assertThat(cw.getPin()).isEqualTo("1234");
            assertThat(cw.getActive()).isTrue();
        }

        @Test
        @DisplayName("Rejeitar vínculo duplicado")
        void rejeitarDuplicado() {
            when(userRepository.findById(clientId)).thenReturn(Optional.of(makeClient()));
            when(userRepository.findById(waiterId)).thenReturn(Optional.of(makeWaiter()));
            when(clientWaiterRepository.existsByClientIdAndWaiterId(clientId, waiterId)).thenReturn(true);

            assertThatThrownBy(() -> waiterService.link(clientId, waiterId, "1234"))
                    .hasMessageContaining("já vinculado");
        }

        @Test
        @DisplayName("Rejeitar se user não é WAITER")
        void rejeitarNaoWaiter() {
            ...
            assertThatThrownBy(() -> waiterService.link(clientId, waiterId, "1234"))
                    .hasMessageContaining("não é um garçom");
        }

        @Test
        @DisplayName("Vincular por email")
        void vincularPorEmail() {
            User waiter = makeWaiter();
            when(userRepository.findByUsername("joao@test.com")).thenReturn(Optional.of(waiter));
            when(userRepository.findById(clientId)).thenReturn(Optional.of(makeClient()));
            when(userRepository.findById(waiterId)).thenReturn(Optional.of(waiter));
            when(clientWaiterRepository.existsByClientIdAndWaiterId(clientId, waiterId)).thenReturn(false);
            when(clientWaiterRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

            ClientWaiter cw = waiterService.linkByEmail(clientId, "joao@test.com", "5678");

            assertThat(cw.getPin()).isEqualTo("5678");
        }

        @Test
        @DisplayName("Validar PIN correto")
        void validarPinCorreto() {
            ClientWaiter cw = ClientWaiter.builder()
                    ...
                    .active(true)
                    .build();
            when(clientWaiterRepository.findByClientIdAndPin(clientId, "1234")).thenReturn(Optional.of(cw));

            ClientWaiter result = waiterService.validatePin(clientId, "1234");
            assertThat(result).isNotNull();
        }

        @Test
        @DisplayName("Rejeitar PIN inválido")
        void rejeitarPinInvalido() {
            when(clientWaiterRepository.findByClientIdAndPin(clientId, "0000")).thenReturn(Optional.empty());

            assertThatThrownBy(() -> waiterService.validatePin(clientId, "0000"))
                    .hasMessageContaining("PIN inválido");
        }
    }
