...
CREATE INDEX idx_delivery_adm_status ON deliveries(adm_id, status) WHERE status != 'CANCELLED';

-- Queries por ADM + data de criação (dashboard de ADM)
CREATE INDEX idx_delivery_adm_created ON deliveries(adm_id, created_at DESC);

-- Queries por Courier + status (app do entregador)
CREATE INDEX idx_delivery_courier_status ON deliveries(courier_id, status) 
    WHERE courier_id IS NOT NULL AND status IN ('ASSIGNED', 'PICKED_UP', 'IN_TRANSIT');

-- Queries por Client + data (histórico do cliente)
CREATE INDEX idx_delivery_client_created ON deliveries(client_id, created_at DESC);

-- Queries por Partnership (relatórios institucionais)
CREATE INDEX idx_delivery_partnership_completed ON deliveries(partnership_id, completed_at DESC) 
...
CREATE INDEX idx_courier_link_primary_active ON courier_adm_links(courier_id) 
    WHERE is_primary = true AND is_active = true;

-- Queries de ADMs por região (via courier)
CREATE INDEX idx_courier_link_adm_active ON courier_adm_links(adm_id, is_active);
...
CREATE INDEX idx_payout_period_status ON unified_payouts(period, status);

-- Pagamentos por registro e tipo (análise financeira)
CREATE INDEX idx_payment_registration_type ON payments(registration_id, payment_type) 
